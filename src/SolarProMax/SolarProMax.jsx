...
export default function SolarProMax() {
    ...
    useEffect(() => {
        const fetchData = () => {
            axios.get('http://localhost:3001/data/solarsync')
                .then(response => {
                    const data = response.data;
                    setSolarData(prevData => ({
                        chart_battery_percentage: [...prevData.chart_battery_percentage, data.battery_percentage].slice(-maxDataPoints),
                        chart_Voltage: [...prevData.chart_Voltage, data.Voltage].slice(-maxDataPoints),
                        chart_Current: [...prevData.chart_Current, data.Current].slice(-maxDataPoints),
                        chart_watt: [...prevData.chart_watt, data.watt].slice(-maxDataPoints),
                        chart_LDR_RIG: [...prevData.chart_LDR_RIG, data.LDR_RIG].slice(-maxDataPoints),
                        chart_LDR_LEF: [...prevData.chart_LDR_LEF, data.LDR_LEF].slice(-maxDataPoints),
                        chart_LDR_TOP: [...prevData.chart_LDR_TOP, data.LDR_TOP].slice(-maxDataPoints),
                        chart_LDR_BOT: [...prevData.chart_LDR_BOT, data.LDR_BOT].slice(-maxDataPoints),
                        chart_angle: [...prevData.chart_angle, data.angle].slice(-maxDataPoints)
                    }));
                })
                .catch(error => {
    ...
    return (
        ...
        <div className='charts'>
                        <div className='chart'>
                            ...
                            <Line data={chartData("battery_percentage", solarData.chart_battery_percentage)} />
                        </div>
                        <div className='chart'>
                            <h2>Voltage</h2>
